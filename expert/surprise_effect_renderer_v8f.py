...
class SurpriseEffectRenderer:
    """
    Manifest-based Surprise renderer.
    ...
    It supports either:
        expert/assets/effects/surprise/manifest.json
    or:
        expert/assets/effects/surprise/surprise/manifest.json
    ...
    Animation:
        ...
        - pulsing purple shock lines
        - continuous orbiting stars and fragments
        - periodic mini flashes
    """

    INTRO_DURATION = 1.15
    MINI_BURST_PERIOD = 2.55
    MINI_BURST_VISIBLE_DURATION = 0.88
    EXCLAMATION_PERIOD = 2.30
    SHOCK_PULSE_PERIOD = 1.90

    INTRO_CORE_COUNT = 4
    INTRO_FRAGMENT_COUNT = 12
    IDLE_CORE_COUNT = 3
    ...
    def __init__(
        ...
        self.exclamation_asset_name: Optional[str] = None
        self.shock_lines_asset_name: Optional[str] = None
    ...
    def _resolve_asset_root(
        ...
        candidates.extend(
            [
                base / "assets" / "effects" / "surprise",
                base / "assets" / "effects" / "surprise" / "surprise",
                base / "assets" / "effects" / "surprise_pack",
            ]
        )
        ...
        for candidate in candidates:
            ...
            expanded.append(candidate / "surprise")
        ...
        raise FileNotFoundError(
            "Cannot find the Surprise sticker pack manifest.\n"
            "Expected one of:\n"
            + "\n".join(checked)
        )
    ...
    def _load_manifest_and_assets(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Cannot read Surprise manifest: {manifest_path}"
            ) from exc
        ...
        if not isinstance(records, list):
            raise ValueError(
                "Surprise manifest must contain an 'assets' list."
            )
        ...
        for record in records:
            ...
            if image is None:
                raise FileNotFoundError(
                    f"Surprise asset is missing: {image_path}"
                )

            if image.ndim != 3 or image.shape[2] != 4:
                raise RuntimeError(
                    "Surprise assets must be RGBA PNGs: "
                    f"{image_path}"
                )
        ...
        if not self.main_asset_name:
            raise ValueError(
                "Surprise manifest does not contain a main sticker."
            )

        if len(self.core_asset_names) < 3:
            raise ValueError(
                "Surprise manifest needs at least 3 core stickers."
            )

        if len(self.fragment_asset_names) < 6:
            raise ValueError(
                "Surprise manifest needs at least 6 fragments."
            )

        self.exclamation_asset_name = next(
            (
                name
                for name in self.core_asset_names
                if "exclamation" in name.lower()
            ),
            None,
        )

        if self.exclamation_asset_name is None:
            self.exclamation_asset_name = next(
                (
                    name
                    for name in self.core_asset_names
                    if "core_01" in name.lower()
                ),
                self.core_asset_names[0]
                if self.core_asset_names
                else None,
            )

        self.shock_lines_asset_name = next(
            (
                name
                for name in self.core_asset_names
                if "shock" in name.lower()
                or "ray" in name.lower()
                or "core_17" in name.lower()
                or "core_16" in name.lower()
            ),
            self.core_asset_names[1]
            if len(self.core_asset_names) > 1
            else None,
        )
    ...
    def _regular_core_pool(self) -> List[str]:
        excluded = {
            self.exclamation_asset_name,
            self.shock_lines_asset_name,
        }
    ...
    def _prepare_activation_layout(self) -> None:
        rng = random.Random(
            20260721 + self.activation_index * 8179
        )

        regular_core = self._regular_core_pool()
        ...
        self.intro_core_particles = self._build_burst_particles(
            ...
            radius_min=0.72,
            radius_max=1.02,
            width_ratio_min=0.16,
            width_ratio_max=0.25,
            duration_min=0.68,
            duration_max=0.90,
            start_angle_degrees=-170.0,
            end_angle_degrees=10.0,
            angle_jitter_degrees=13.0,
        )
        ...
        self.intro_fragment_particles = self._build_burst_particles(
            ...
            radius_min=0.82,
            radius_max=1.16,
            width_ratio_min=0.050,
            width_ratio_max=0.088,
            duration_min=0.74,
            duration_max=1.02,
            start_angle_degrees=-178.0,
            end_angle_degrees=18.0,
            angle_jitter_degrees=9.0,
        )
        ...
        self.core_floaters = self._build_floaters(
            ...
            width_ratio_range=(0.11, 0.16),
            orbit_range=(0.035, 0.060),
            opacity_range=(0.60, 0.92),
        )
        ...
        self.fragment_floaters = self._build_floaters(
            ...
            width_ratio_range=(0.040, 0.072),
            orbit_range=(0.025, 0.050),
            opacity_range=(0.42, 0.80),
        )
    ...
    def _transform_rgba(
        ...
        height = max(
            1,
            int(round(source_height * width / source_width)),
        )
        ...
        matrix[0, 2] += (
            bound_width / 2.0 - center[0]
        )
        matrix[1, 2] += (
            bound_height / 2.0 - center[1]
        )
    ...
    def _overlay_rgba(
        ...
        x1 = int(round(center_x - overlay_width / 2.0))
        y1 = int(round(center_y - overlay_height / 2.0))
        x2 = x1 + overlay_width
    ...
    def _draw_cloud(
        ...
        pop = self._ease_out_back(entry_progress)
    ...
    def _draw_burst(
        ...
        for particle in particles:
            ...
            progress = self._smoothstep(
                particle_time / particle.duration
            )
            ...
            center_x = (
                origin_x
                + particle.direction_x * travel
            )
            center_y = (
                origin_y
                + particle.direction_y * travel
            )

            sprite_width = int(
                max(
                    11,
                    face_width * particle.width_ratio,
                )
            )
    ...
    def _draw_exclamation(
        ...
        if intro:
            local_time = elapsed - 0.08

            if local_time < 0.0 or local_time > 0.95:
                return

            progress = self._ease_out_back(
                min(1.0, local_time / 0.44)
            )
            ...
            cycle = (
                elapsed % self.EXCLAMATION_PERIOD
            )
        ...
        self._overlay_rgba(
            ...
            center_x,
            center_y,
            max(0.0, local_opacity) * opacity,
        )

    def _draw_shock_lines(
        self,
        frame: np.ndarray,
        face_box: FaceBox,
        elapsed: float,
        ...
        if self.shock_lines_asset_name is None:
            return

        x, y, width, height = face_box

        if intro:
            local_time = elapsed - 0.10

            if local_time < 0.0 or local_time > 0.88:
                return
            ...
            cycle = (
                elapsed % self.SHOCK_PULSE_PERIOD
            )
    ...
    def _periodic_burst_layout(
        ...
        fragment_particles = self._build_burst_particles(
            ...
            radius_min=0.53,
            radius_max=0.80,
            width_ratio_min=0.040,
            width_ratio_max=0.068,
            duration_min=0.62,
            duration_max=0.82,
        ...
        return core_particles, fragment_particles
    ...
    def render(
        ...
        intro = elapsed <= self.INTRO_DURATION

        self._draw_shock_lines(
            frame,
            face_box,
            elapsed,
            opacity,
